...
func validateDeploymentOnCluster(cluster *framework.Cluster, name types.NamespacedName) error {
	deployment := &appsv1.Deployment{}
	if err := cluster.KubeClient.Get(ctx, name, deployment); err != nil {
		return err
	}

	// Use the object created in the hub cluster as reference.
	wantDeployment := &appsv1.Deployment{}
	if err := hubClient.Get(ctx, name, wantDeployment); err != nil {
		return err
	}
	if diff := cmp.Diff(
		...
		return fmt.Errorf("app deployment %s diff (-got, +want): %s", name.Name, diff)
	}
...
func deploymentPlacedOnClusterActual(cluster *framework.Cluster) func() error {
	workNamespaceName := fmt.Sprintf(workNamespaceNameTemplate, GinkgoParallelProcess())
	return func() error {
		return validateDeploymentOnCluster(cluster, types.NamespacedName{Name: fmt.Sprintf(appDeploymentNameTemplate, GinkgoParallelProcess()), Namespace: workNamespaceName})
	}
}
