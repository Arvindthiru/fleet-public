...
var _ = Describe("validating CRP when selected resources cross the 1MB limit", Ordered, func() {
	...
	BeforeAll(func() {
		By("ensure fleet validating webhook configuration exists and it works")
		ensureFleetValidatingWebhookConfigurationExists()

		By("creating large resources")
		resourcesForMultipleResourceSnapshots()
	...
	It("should place the selected resources on member clusters", func() {
		...
		checkIfPlacedDeploymentResourceOnAllMemberCluster()
	})
